"""
Working directory for all local changes:
    ~/Portfolio/Graph Theory/
    
First version of base simulation.
Remnants of visualisation commented out, with the exception of simulation time printing at the end to demonstrate that it runs properly.
Visualisation component to be added to viz later.
Save all data to pkl, such that it can be read by vis.
Choose time interval 20 steps, run each simulation for 200 steps.
Every 20 steps, write to pkl.
Each .pkl should contain data for all spawn rates in 20 step intervals.

Data:
    Agents currently on grid
    Total spawned
    Actual spawns in the current time step
    Total successful traversals
    Successful traversals in the current time step
"""
...
#edit this to make functional on personal machine
root = "./bin"
prefix = "chunk_"

#counter = integer from 1 to chunk_count as defined below
def save(data, spawn_rate, counter=1):
    name = prefix + f"{spawn_rate}_" + f"{counter}.pkl"
...
def navigate_graph(seed=0, spawn_rate = 1):    
    ...
    print("Navigating Graph with spawn rate {}".format(spawn_rate))
    
    on_grid, number_of_spawns_total, spawns_current_step, off_grid, off_grid_current_step = [], [], [], [], []
    ...
    new_agent = []
    """
    In below loop, every t time steps write all data found in the current chunk to pkl labelled chunk_n, where n is an iteration counter.
    Data structure is DoL: {[a],[b],[c],[d],[e]}
    Each parameter is a list of t data points for each of the 5 measurements to be obtained.
    Currently, overwrites previous chunk instead of adding to it. Need to either add to chunk, or switch data form.
    """
    for current_time in np.arange(0, time_limit):
        ...
        #write to pkl if time is multiple of chunk_size
        if (current_time+1) % chunk_size == 0:
            
            """
            write to pkl here
            """
            data = {"Ongrid":on_grid,"SpawnTotal": number_of_spawns_total,"SpawnCurrent":spawns_current_step,"Offgrid":off_grid,"OffgridCurrent":off_grid_current_step}
            
            save(data, spawn_rate, counter)
...
while spawn_rate <= 2:
    ...
    print("Data for Spawn Rate {} generated.".format(spawn_rate))
