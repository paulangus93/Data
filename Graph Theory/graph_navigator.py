...
#Set locations of initial and goal nodes in the graph G.
# def initial_goal_nodes(G): 
    
#     initial_nodes = [(0,1),(0,2),(0,3),(0,4),(0,0)]
    
#     goal_nodes = [(9,14),(8,14),(7,14),(6,14),(5,14)]
...
def move(ID, path, current_node, new_agent, exclusion, travelled_paths):  
    ...
    #if agent has not spawned this time step
    if ID not in new_agent:
...
def recalibrate_path(G, ID, occupancy, paths, initial_nodes, exclusion):
    ...
    if valid_nodes == []:
        ...
        print("Agent {} must wait due to no valid path.".format(ID))
        ...
        try:
            ...
            print("Agent {} must wait.".format(ID))
...
def move_all_agents(G, paths, occupancy, agents_complete, all_agents_complete, initial_nodes, goal_nodes, new_agent, exclusion, travelled_paths):
    ...
    agents_to_move = [x for x in paths.keys() if x != new_agent]

             
    #until agents_to_move is empty
...
def navigate_graph(seed=0, spawn_rate = 1):    
    ...
    for current_time in np.arange(0, time_limit):     
        ...
        animatetime0 = time.time()
        animate(G, occupancy, initial_nodes, goal_nodes)
        animatetime1 = time.time()
        animatetime.append(animatetime1-animatetime0)
    ...
    print(on_grid)
    print(number_of_spawns_total)
    print(spawns_current_step)
    print(off_grid)
    print(off_grid_current_step)
...
"""
Future revisions:
    3 elements to a grid:
inflow rate, outflow rate, grid capacity.
    Control inflow rate for a given size, track:
outflow rate, number of agents, time spent waiting

I would suggest start logging your data - agent on grid, agents added agents removed as a function of time.
Also for next week suggest reading some of those papers. Lets see if we can think up of a better routing algo (all other thinsg beign a constant) 
For next weeks catch up - I'll be looking for (a) graphs indicating the metrics I indicated - stick to 10x15 - vary spawn rate and (b) i want you to talk through the logic around an algorithm you think looks worth coding up

OPTIONAL: Try fixing spawn function, but in the meantime just track spawn numbers.
"""

#%%
"""
Working Version with A* pathfinding. Do not alter or delete.
"""
...
"""
Implement Reservation Lists.
Reservation is a dictionary of {agent: [(time,node), (time2,node2)...]}
Priority based purely on spawn order.
Replace Exclusion consideration in the Move step with Reservation consideration in the Spawn step.
"""
...
def navigate_graph(seed=0, spawn_rate = 1):    
    ...
    for current_time in np.arange(0, time_limit):     
        ...
        #spawn 
        spawntime0=time.time()
        new_agent, ID, reservation = spawn(ID, spawn_rate, initial_nodes, goal_nodes, occupancy, paths, travelled_paths, exclusion, reservation, current_time)
        spawntime1=time.time()
        spawntime.append(spawntime1-spawntime0)
        
        
        #move all agents that are currently on grid and not recently spawned.
        movetime0 = time.time()
        move_all_agents(G, paths, occupancy, agents_complete, all_agents_complete, initial_nodes, goal_nodes, new_agent, exclusion, travelled_paths)
        movetime1=time.time()
        movetime.append(movetime1-movetime0)
...
"""
Next stages:
    Go back to previous version (this version's changes will not be wasted as they can be iterated on later), and separate it into defined core and visualisation sections. Ensure good commenting practice and commit them to Kiran's repository in the next few days.
